...
parser.add_argument("--file_name", type=str, default="1-deconv/adaptiveLU/", help="saving directory")
...
""" NETWORK SETUP """
netA = nets.netA_kernel(args, nlayers=3).to(args.device)
netR = nets.proxNet_deconv(args, nlayers=9).to(args.device)
...
if args.train:
    ...
    for epoch in range(args.n_epochs):
        ...
        with tqdm(total=(tr_length - tr_length % args.batch_size)) as _tqdm:
            ...
            if (epoch + 1) % 10 == 0:
                plot_reflectivity(X, X0, Zk, args, epoch)
    ...
    with tqdm(total=(ts_length - ts_length % args.batch_size)) as _tqdm:
        ...
        for y, X, A0 in ts_loader:
            ...
            A0_meters.update(criteria(y, A0 @ X).item(), bs)
