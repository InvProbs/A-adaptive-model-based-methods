...
parser.add_argument('--lr_X', type=float, default=1e-4, help='step size for updating X')
...
parser.add_argument("--file_name", type=str, default="4-deblur/adaptiveLU/", help="saving directory")
...
""" NETWORK SETUP """
netA = nets.netA_deblur(args, nlayers=3).to(args.device)
netR = nets.proxNet_deblur(args, nlayers=5).to(args.device)
...
if args.train:
        ...
        with tqdm(total=(tr_length - tr_length % args.batch_size)) as _tqdm:
            ...
            for X, _ in tr_loader:  # X, _ = next(iter(tr_loader))
                ...
                for k in range(args.maxiters):
                    ...
                    yinit, yk = netA(Xk, A0)
                    optX.zero_grad()
                    lossX = criteria(y, yk) + 0.01 * criteria(yk, yinit) + 0.1 * criteria(Zk.detach(), Xk)
                    # lossX = criteria(y, yk) + 0.1 * criteria(Zk.detach(), Xk)
                    lossX.backward()
                    optX.step()
                    Xk = Xk.detach()
                    ...
                    yinit, yk = netA(Xk, A0)
                    optA.zero_grad()
                    lossA = criteria(y, yk) + 0.01 * criteria(yk, yinit)
            ...
            if (epoch + 1) % 10 == 0:
                plot_deblur(X, X0, Zk, args, epoch, algo="AdaptiveLU")
    ...
    with tqdm(total=(ts_length - ts_length % args.batch_size)) as _tqdm:
        ...
        for X, _ in ts_loader:
            ...
            A0X = A0(X)
            A0_meters.update(criteria(y, A0X).item(), bs)
            for k in range(args.maxiters):
                """ UPDATE X  plt.figure();plt.imshow(Xk[0,0].cpu()) """
                netA.eval()
                netR.eval()
            ...
            avg_init_psnr, avg_recon_psnr, avg_delta_psnr, avg_ssim = compute_metrics1chan(Zk, X, y)
            ...
            dict.update({f'A0': f'{A0_meters.avg:.6f}', f'A_1iter': f'{A_meters_1iter.avg:.6f}',
                         f'A_Kiter': f'{A_meters_Kiters.avg:.6f}'})
            _tqdm.set_postfix(dict)
            _tqdm.update(bs)
    ...
    plot_deblur(X, X0, Zk, args, -1, algo="AdaptiveLU")